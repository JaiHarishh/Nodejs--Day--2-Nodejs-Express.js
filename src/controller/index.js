...
export function bookingStatus(roomName){
    return client.db("Hall").collection("booking").findOne({room_name:roomName})
}

//to update the status as booked
export function statusUpdation(id,data){
    return client.db("Hall").collection("booking").updateOne({_id:new ObjectId(id)},{$set:data})
}

//to get all the booked rooms
export function getAllBookedRooms(){
    return client.db("Hall").collection("booking").find({booking_status:"Booked"}).toArray();
}

//to get all the rooms that are booked
export function checkBooking(){
    return client.db("Hall").collection("booking").find({})
}
